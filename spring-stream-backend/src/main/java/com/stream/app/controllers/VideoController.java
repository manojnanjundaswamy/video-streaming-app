...
public class VideoController {
    ...
    // video upload
    @PostMapping
    public ResponseEntity<?> create(@RequestParam("file") MultipartFile file, @RequestParam("title") String title, @RequestParam("description") String description, @RequestParam("thumbnailFile") MultipartFile thumbnailFile) {
        ...
        Video savedVideo = videoService.save(video, file, thumbnailFile);
...
//    http://localhost:8080/api/v1/videos/stream/35212512
    ...
    public ResponseEntity<Resource> getThumbnail(@PathVariable String videoId) {
        ...
        String filePath = video.getThumbnailPath();
        Resource resource = new FileSystemResource(filePath);
        if (contentType == null) {
    ...
    public ResponseEntity<Resource> streamVideoRange(@PathVariable String videoId, @RequestHeader(value = "Range", required = false) String range) {
        ...
        //If no range headers are present, then return the full file
        if (range == null) {
            return ResponseEntity.ok().contentType(MediaType.parseMediaType(contentType)).body(resource);
        }
